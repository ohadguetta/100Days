...
var end = new Date('06/23/2024');
var now = new Date();
var time = end - now;
var days = Math.floor(time / (1000 * 60 * 60 * 24)) + 1;
...
class Sketch{
	...
	createWall(){
		var wall1=   Bodies.rectangle(this.width/2, this.height+30, 1,1, { isStatic: true });
	...
	createBodies(){
		...
		for(var i = 0; i < days; i++){
			...
			div.classList.add("physicsDiv");
			div.innerHTML = `<img src="teaching.png">`;
		...
		htmlElems.map(elem=>{
			...
			bod.update = ()=>{
				...
				//Body.applyForce(bod,bod.position,Matter.Vector.mult( Matter.Vector.sub(Matter.Vector.create(this.width/2,this.height/2),bod.position),0.0003));
